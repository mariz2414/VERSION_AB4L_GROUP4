...
<?php
	...
	function insert($query){	
		...
		$link = oci_connect('ONLINE AUCTION','uplbonlineauction');



		if (!$link) { 

			die('Could not connect to MySQL: ' . oci_error()); 
	...
	function checkExistencePK($uniqueKey,$table){
		...
		$connect = oci_connect('ONLINE AUCTION','uplbonlineauction');
...
<body>
	...
	<form name="createaccount" action = "auction.php?page=success" onsubmit="return validateForm();" method="POST">
		...
		<table>
			...
			<tr>
				...
				<td><input type="text" id="home_no" name="home_no" size="30" placeholder="e.g. 000-0000"></td>
			...
			<tr>
				...
				<td><textarea rows="6" cols="23" id="interests" name="interests" placeholder="List of things that you are interested in buying"></textarea></td>
			...
			<tr>
				...
				<td colspan="2">By creating an account and clicking Submit, you agree to the rules that this site has set and the equivalent consequences of violating them.</td>
	...
	//inserting into database
		...
		if(isSet($_POST['create'])&&isSet($_POST['studnum'])&&isSet($_POST['lname'])&&isSet($_POST['fname'])&&isSet($_POST['mname'])&&isSet($_POST['emailadd'])){
			...
			if(!checkExistencePK($_POST['studnum'],'ACCOUNT')){
				...
				echo "<script>alert('Account creation unsuccessful. The student number you entered already has an account!'); window.location = 'auction.php#' </script>";
